...
# limix-ext __init__ can detect if it is being loaded by the setup routine, to
# avoid attempting to load components that aren't built yet.  While ugly, it's
# a lot more robust than what was previously being used.
builtins.__LIMIX_EXT_SETUP__ = True
        ...
        print(textwrap.dedent("""
            ...
            with pip instead of using `setup.py install`:
              - `pip install .`       (from a git repo or downloaded source
                                       release)
              - `pip install limix-ext`   (last limix-ext release on PyPi)
            """))
        return True

    if '--help' in sys.argv[1:] or '-h' in sys.argv[1]:
        print(textwrap.dedent("""
        limix-ext-specific help
        -------------------
        To install limix-ext from here with reliable uninstall, we recommend
        that you use `pip install .`. To install the latest limix-ext release
        from PyPi, use `pip install limix-ext`.
...
def get_test_suite():
    ...
    return TestLoader().discover('limix_ext')
...
def configuration(parent_package='', top_path=None):
    ...
    config.add_subpackage('limix_ext')

    config.get_version('limix_ext/version.py') # sets config.version
