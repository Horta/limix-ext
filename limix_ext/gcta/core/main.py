...
def _create_their_kinship(bed_folder, prefix):
    ...
    if platform() != 'linux':
        raise OSError(
            'The %s platform is currently not supported.' % platform())
    ...
    cmd = [
        './gcta64', '--bfile',
        os.path.join(bed_folder, prefix), '--autosome', '--make-grm', '--out',
        os.path.join(bed_folder, prefix)
    ]
...
def _run_gcta(prefix, phen_filename, preva, diag_one=False, nthreads=1):
    ...
    cmd = [
        './gcta64', '--grm', prefix, '--pheno', phen_filename, '--prevalence',
        '%.7f' % preva, '--reml', '--out',
        os.path.join(outfolder, 'result'), '--thread-num',
        str(nthreads)
    ]
    if diag_one:
...
def run_gcta(bed_folder, prefix, prevalence, diag_one=False):
    ...
    result = _run_gcta(
        os.path.join(bed_folder, prefix),
        os.path.join(bed_folder, phen_filename),
        prevalence,
        diag_one=diag_one,
        nthreads=1)
    return result
